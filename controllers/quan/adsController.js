...
const showDetail = (req, res) => {
	...
	switch (category) {
		case 'spot':
			res.render('spot-detail', {url: req.originalUrl, title: 'Quận - Chi tiết điểm đặt', toolbars: toolbars});
			break;
		case 'board':
			res.render('board-detail', {url: req.originalUrl, title: 'Quận - Chi tiết bảng quảng cáo', toolbars: toolbars});
			break;
		default:
			res.status(404);
			return res.render('error', {error: {status: 404, message: 'Không tìm thấy trang'}});
...
const showAdd = (req, res) => {
	...
	switch (category) {
		case 'spot':
			res.render('spot-new', {url: req.originalUrl, title: 'Quận - Điểm đặt mới', toolbars: toolbars});
			break;
		case 'board':
			res.render('board-new', {url: req.originalUrl, title: 'Quận - Bảng quảng cáo mới', toolbars: toolbars});
			break;
		default:
			res.status(404);
			return res.render('error', {error: {status: 404, message: 'Không tìm thấy trang'}});
...
const showModify = (req, res) => {
	...
	switch (category) {
		case 'spot':
			res.render('spot-modify', {url: req.originalUrl, title: 'Quận - Chỉnh sửa điểm đặt', toolbars: toolbars});
			break;
		case 'board':
			res.render('board-modify', {url: req.originalUrl, title: 'Quận - Chỉnh sửa bảng quảng cáo', toolbars: toolbars});
			break;
		default:
			res.status(404);
			return res.render('error', {error: {status: 404, message: 'Không tìm thấy trang'}});
